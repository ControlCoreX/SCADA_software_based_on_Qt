...
@verbatim  
 ...
 [..]             
   (#) Enable the GPIO AHB clock using the following function
       RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOx, ENABLE);											用这个函数使能AHB时钟
               
   (#) Configure the GPIO pin(s) using GPIO_Init()																配置GPIO引脚的模式函数
   ...
   (#) Peripherals alternate function:																						外设复用功能配置的函数
   ...
   (#) To get the level of a pin configured in input mode use GPIO_ReadInputDataBit()	为了读出引脚的电平要用这个函数
            
   (#) To set/reset the level of a pin configured in output mode use									引脚置/复位函数 
       GPIO_SetBits()/GPIO_ResetBits()
                 
   (#) During and just after reset, the alternate functions are not 								使用复位函数之后，这个引脚的复用功能就不能用了
       active and the GPIO pins are configured in input floating mode (except JTAG	而且被配置成了浮空模式
       pins).
...
/** @addtogroup STM32F4xx_StdPeriph_Driver					告诉你这个文件添加到哪一组
  * @{
  */
...
/**
  ...
  * @note   By default, The GPIO pins are configured in input floating mode (except JTAG pins).			初始化后，引脚变成浮空输入模式
  * @param  GPIOx: where x can be (A..K) to select the GPIO peripheral for STM32F405xx/407xx and STM32F415xx/417xx devices
  *                      x can be (A..I) to select the GPIO peripheral for STM32F42xxx/43xxx devices.
  *                      x can be (A, B, C, D and H) to select the GPIO peripheral for STM32F401xx devices.  
  * @retval None
  */
void GPIO_DeInit(GPIO_TypeDef* GPIOx)			//GPIO默认初始化，其中的De是default
{
